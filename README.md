# JOYFIT24 経堂 9周年チャレンジ

プロジェクト名: `JOYFIT24-KYODO-9th-event`

## 内容

- `index.html`: スマホ向けのログイン制投稿フォーム兼ランキングページです。
- `gas/Code.gs`: Google Apps Scriptに貼り付けるスプレッドシート連携コードです。

公開URL（Vercel）: https://joyfit-9th-event.vercel.app

## 運用イメージ

1. 参加者はQRからページを開き、ニックネーム、4桁パスワードでログインします。
2. 初回ログイン時に `参加者` シートへ自動登録されます。
3. ログイン後、今週の種目の記録だけ入力して送信します。
4. 送信データは種目別シート（例: `握力測定`）に入り、ランキングに反映されます。

## 画面の使い方

1. 「ログイン」でニックネーム、4桁パスワードを入力します。
2. ログイン後、記録を入力して送信します。
3. 「ランキング」で今週TOP10と過去WeekのTOP10を確認できます。

## GAS連携手順

1. Googleスプレッドシートを開きます。
2. `拡張機能 > Apps Script` を開きます。
3. `gas/Code.gs` の中身を貼り付けて保存します。
4. エディタ上部の関数選択で `setupSheets` を選んで実行します。
5. 初回だけGoogleの権限許可を行います。
6. 次のシートが作成（または旧英語名からリネーム）されます。
   - `参加者`
   - `セッション`
   - `握力測定` / `前屈` / `プランク` / `腕立て伏せ`
7. `参加者` には動作確認用として `テスト / 1111` と `STAFF / 9999` が入ります。通常は参加者の初回ログインで自動追加されます。
8. `デプロイ > 新しいデプロイ > ウェブアプリ` を選びます。
9. 実行ユーザーは「自分」、アクセス権は **「全員」**（Googleアカウント不要）にします。「Googleアカウントを持つ全員」だと、PCでは動いてスマホだけ失敗しやすいです。
10. 発行されたウェブアプリURLを `index.html` の `CONFIG.gasEndpoint` に貼り付けます。

既存デプロイを更新する場合は、`デプロイを管理 > 編集 > バージョン: 新しいバージョン` を選んで再デプロイしてください。保存だけでは公開URL側に反映されません。

```js
const CONFIG = {
  gasEndpoint: "https://script.google.com/macros/s/xxxxxxxxxxxxxxxx/exec"
};
```

このページは、公開サイト上では Vercel の `/api/gas` 経由で GAS と通信します（スマホの LINE 内ブラウザ等でもつながりやすくするため）。ローカル確認時は従来どおり JSONP にフォールバックします。

GAS のアクセス権は「全員」（Googleアカウント不要）にしてください。PCではGoogleログイン済みで動いても、スマホでは失敗することがあります。

今回のスプレッドシートIDは `Code.gs` の `SPREADSHEET_ID` に設定済みです。

旧シート名（`participants` / `sessions` / `records`）がある場合:

- `participants` → `参加者`、`sessions` → `セッション` へ自動リネームします。
- 旧 `records` の行データは、種目シートが空のときだけ 1人1行形式へ自動移行します。

## 参加者 シート

店舗側で確認できる参加者管理シートです。

`participantId`, `nickname`, `pin`, `division`, `active`, `memo`, `createdAt`, `updatedAt`

- `participantId`: 自動発行される内部IDです。
- `nickname`: ランキング表示名です。個人が特定されない名前を案内してください。
- `pin`: 4桁パスワードです。例: `1234`
- `division`: `member` または `staff`
- `active`: `TRUE` ならログイン可能

## 種目シート（握力測定 / 前屈 / プランク / 腕立て伏せ）

1人あたり1行で、最大3回分の記録を横持ちします。

| 列 | 項目 | 内容 |
|---|---|---|
| A | participantId | 内部ID |
| B | createdAt | 初回登録日時 |
| C | updatedAt | 最終更新日時 |
| D | division | member / staff |
| E | displayName | 表示名 |
| F | unit | 単位 |
| G | attempts | 登録回数（1〜3） |
| H | score1 | 1回目の記録 |
| I | score2 | 2回目の記録 |
| J | score3 | 3回目の記録 |
| K | date1 | 1回目の日付 |
| L | date2 | 2回目の日付 |
| M | date3 | 3回目の日付 |

ランキングは `score1 + score2 + score3` の合計です。

## テストWeek / スタッフレビュー期間

日付判定で週を切り替えます（`testWeekOverride` / `TEST_WEEK_OVERRIDE` は未使用）。

スタッフレビューのため、第1週（握力測定）だけ開始を前倒ししています。

| 週 | 種目 | 期間 |
|---|---|---|
| 1 | 握力測定 | 2026/7/20 〜 2026/8/9 |
| 2 | 前屈 | 2026/8/10 〜 2026/8/16 |
| 3 | プランク | 2026/8/17 〜 2026/8/23 |
| 4 | 腕立て伏せ | 2026/8/24 〜 2026/8/30 |

本番前にテスト参加者データをスプレッドシートから消してください。第1週の開始日を本番日程に戻す場合は、`index.html` と `Code.gs` の両方を揃えて更新し、GASは新バージョンで再デプロイしてください。

## 現在のルール

- 開催期間（レビュー含む）は 2026/7/20 から 2026/8/30 です。
- Week 1: 握力測定
- Week 2: 前屈
- Week 3: プランク
- Week 4: 腕立て伏せ
- 同じ識別情報は1週間に最大3回まで登録できます。
- 同じ識別情報は同じ日に1回まで登録できます。
- ランキングは同じ識別情報の週内記録を合計して並べます。

## 調整しやすいところ

- イベント日程や種目名は `index.html` と `Code.gs` の `weeks` / `EVENT_WEEKS` を変更します。
- スタッフをランキングから分けたい場合は、`division` を使って表示条件を追加できます。
